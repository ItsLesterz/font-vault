// src/data/fontSites.js
// Add, remove, or edit entries here to manage the repository.

export const CATEGORIES = [
  { id: "all",        label: "All",              icon: "bi-grid-fill" },
  { id: "serif",      label: "Serif",            icon: "bi-type" },
  { id: "sans-serif", label: "Sans-serif",       icon: "bi-fonts" },
  { id: "display",    label: "Display",          icon: "bi-stars" },
  { id: "script",     label: "Script / Handwriting", icon: "bi-pen" },
  { id: "monospace",  label: "Monospace",        icon: "bi-code-slash" },
  { id: "variable",   label: "Variable Fonts",   icon: "bi-sliders" },
];

export const PRICING_TYPES = ["Free", "Paid", "Free + Paid"];

export const FONT_SITES = [
  {
    id: 1,
    name: "FontSquirrel",
    url: "https://www.fontsquirrel.com/",
    description: "Un directorio de fuentes curado con una amplia selección de tipografías para explorar y descargar.",
    featured: true,
  },
  {
    id: 2,
    name: "Google Fonts",
    url: "https://fonts.google.com/",
    description: "La biblioteca de fuentes de código abierto más grande, con muchas tipografías de calidad para web e impresión.",
    featured: true,
  },
  {
    id: 3,
    name: "Adobe Fonts",
    url: "https://fonts.adobe.com/",
    description: "Una amplia biblioteca de fuentes integrada con Adobe Creative Cloud para uso en web y escritorio.",
    featured: true,
  },
  {
    id: 4,
    name: "FontSpring",
    url: "https://www.fontspring.com/",
    description: "Un mercado profesional de fuentes con una gran colección de tipografías para proyectos personales y comerciales.",
    featured: true,
  },
  {
    id: 5,
    name: "DaFont",
    url: "https://www.dafont.com/es/",
    description: "Un sitio popular con muchas fuentes decorativas, temáticas y de exhibición para diferentes estilos.",
    featured: true,
  },
  {
    id: 6,
    name: "FontSource",
    url: "https://fontsource.org/",
    description: "Un recurso de fuentes de código abierto enfocado en facilitar el autoalojamiento y la integración en desarrollo web.",
    featured: true,
  },
  {
    id: 7,
    name: "Fontke",
    url: "https://eng.fontke.com/",
    description: "Un sitio web de fuentes con diferentes estilos tipográficos para explorar y descargar.",
    featured: true,
  },
  {
    id: 8,
    name: "Dafont Free",
    url: "https://www.dafontfree.io/",
    description: "Un sitio con una amplia colección de fuentes descargables para distintos estilos y usos.",
    featured: true,
  },
  {
    id: 9,
    name: "Fontyukle",
    url: "https://www.fontyukle.net/download",
    description: "Un sitio de descarga de fuentes con varias opciones tipográficas para navegar y obtener.",
    featured: true,
  },
  {
    id: 10,
    name: "1001 Fonts",
    url: "https://www.1001fonts.com/",
    description: "Un directorio de fuentes con una gran variedad de tipografías para proyectos creativos y digitales.",
    featured: true,
  },
  {
    id: 11,
    name: "YouWorkForThem",
    url: "https://www.youworkforthem.com/",
    description: "Una plataforma con fuentes y recursos gráficos orientados a diseñadores y proyectos creativos.",
    featured: true,
  },
  {
    id: 12,
    name: "Open Foundry",
    url: "https://open-foundry.com/",
    description: "Un sitio curado de fuentes con una selección variada para explorar y usar en proyectos digitales.",
    featured: true,
  },
  {
    id: 13,
    name: "Lost Type Co-op",
    url: "https://www.losttype.com/",
    description: "Una fundición digital con una colección de tipografías para branding, editorial y diseño creativo.",
    featured: true,
  },
  {
    id: 14,
    name: "Future Fonts",
    url: "https://www.futurefonts.com/",
    description: "Una plataforma de fuentes en desarrollo donde se publican tipografías nuevas y experimentales.",
    featured: true,
  },
  {
    id: 15,
    name: "Type Foundry Directory",
    url: "https://typefoundry.directory/",
    description: "Un directorio de fundiciones tipográficas para descubrir estudios y catálogos de fuentes.",
    featured: true,
  },
  {
    id: 16,
    name: "FFonts",
    url: "https://ffonts.net/",
    description: "Un sitio de fuentes con una amplia variedad de tipografías para descargar y usar en proyectos creativos.",
    featured: true,
  }
];
